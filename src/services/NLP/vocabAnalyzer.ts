...
class VocabularyAnalyzerService {
    ...
    try {
      ...
      if (!lemmatizeWord) {
        try {
          ...
          if (mod && typeof (mod as any).verb === 'function') {
            lemmatizeWord = (w: string) => {
              const lower = w.toLowerCase();
              const v = (mod as any).verb(lower);
              if (v !== lower) return v;
              const n = (mod as any).noun(lower);
              if (n !== lower) return n;
              return (mod as any).adjective(lower);
            };
          }
        } catch (e) {
      ...
      let baseScore = levelScores[overallLevel as keyof typeof levelScores] || 65;
      ...
      if (penaltyPoints > 20) penaltyPoints = 20;
  ...
  private generateSuggestions(
    ...
    for (const [base, synonyms] of Object.entries(CEFR_SYNONYM_LOOKUP)) {
      if (synonymHintCount >= 3) break; // Suggest up to 3 synonyms
      ...
      if (unusedSynonyms.length === 0) continue;

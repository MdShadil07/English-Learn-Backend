...
export class ComprehensiveJobScheduler {
    ...
    start() {
        ...
        const dailyStreakValidation = cron.schedule('0 0 * * *', async () => {
            try {
        ...
        const streakHealthCheck = cron.schedule('0 */6 * * *', async () => {
            try {
        ...
        const weeklyStreakAnalytics = cron.schedule('0 2 * * 0', async () => {
            try {
        ...
        // ============================================
        // Hourly - Update accuracy trending data
        const hourlyAccuracyTrends = cron.schedule('0 * * * *', async () => {
            try {
        ...
        const dailyWeeklyAnalytics = cron.schedule('0 2 * * *', async () => {
            try {
        ...
        const batchedProgressFlush = cron.schedule('*/30 * * * *', async () => {
            try {
        ...
        const cacheCleanup = cron.schedule('*/15 * * * *', async () => {
            try {
        ...
        // Every hour - Update leaderboard cache
        const leaderboardUpdate = cron.schedule('0 * * * *', async () => {
            try {
        ...
        const databaseOptimization = cron.schedule('0 3 * * *', async () => {
            try {
        ...
        const statsReporting = cron.schedule('0 */12 * * *', () => {
            logger.info({
     ...
     */
    initializeAccuracyQueue() {
        if (this.accuracyQueue) {
            return;
        }
        // Create queue
        const queue = new Queue('accuracy-analysis', {
        ...
        // Create worker
        const worker = new Worker('accuracy-analysis', async (job) => this.handleAccuracyJob(job), {
            connection: queueRedis,
    ...
    async handleAccuracyJob(job) {
        ...
        try {
            ...
            if (xpResult.totalXP !== 0) {
                await progressOptimizationService.addXP(userId, xpResult.totalXP, xpResult.totalXP >= 0 ? 'ai_chat' : 'penalty', 'conversation', { immediate: true });
            }
     ...
     */
    async queueAccuracyAnalysis(data) {
        if (!this.accuracyQueue) {
            this.initializeAccuracyQueue();
        }
        if (!this.accuracyQueue) {
     ...
     */
    async runWeeklyAnalyticsForAllUsers() {
        try {
...
//# sourceMappingURL=comprehensiveJobScheduler.js.map
 ...
 */
export const updateAccuracyMiddleware = async (req, res, next) => {
    try {
        ...
        const userId = res.locals.userId || req.body.userId || req.user?.id;
        ...
        await progressOptimizationService.updateAccuracyData(userId, accuracyData, { priority: 'medium' });
 ...
 */
export const addXPMiddleware = async (req, res, next) => {
    try {
        ...
        await progressOptimizationService.addXP(userId, xpAmount, resolvedSource, category, { immediate: false });
        next();
    }
    catch (error) {
 ...
 */
export const handleLevelUpMiddleware = async (req, res, next) => {
    try {
        const levelUpData = res.locals.levelUpData;
        const userId = res.locals.userId || req.user?.id;
        if (!levelUpData || !userId) {
            next();
            return;
        }
        // Level-up is always immediate
        await progressOptimizationService.updateLevel(userId, levelUpData.newLevel, levelUpData.rewards);
        next();
    }
    catch (error) {
 ...
 */
export const invalidateCacheMiddleware = (dataType) => {
...
//# sourceMappingURL=progressUpdate.middleware.js.map
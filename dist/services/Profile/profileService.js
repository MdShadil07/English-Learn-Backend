...
export class ProfileService {
     ...
     */
    async updateProfile(userId, updateData, updatedBy) {
        try {
            ...
            try {
                ...
                const { education, certifications, ...profileFields } = updateData;
                const baseUpdate = {
                    ...profileFields,
                ...
                const profileUpdate = await UserProfile.findOneAndUpdate({ userId: userObjectId }, { $set: baseUpdate }, {
                    new: true,
                    runValidators: true,
                    session,
                    upsert: true,
     ...
     */
    async batchUpdateProfiles(updates, updatedBy) {
        try {
            const session = await UserProfile.startSession();
            session.startTransaction();
            try {
     ...
     */
    async updateEducation(userId, education, session) {
        // Remove existing education
        await UserProfile.updateOne({ userId: new Types.ObjectId(userId) }, { $unset: { education: 1 } }, { session });
        // Add new education if provided
        if (education.length > 0) {
            await UserProfile.updateOne({ userId: new Types.ObjectId(userId) }, {
                $push: {
                    education: {
                        $each: education.map((edu) => ({
                            ...edu,
     ...
     */
    async updateCertifications(userId, certifications, session) {
        // Remove existing certifications
        await UserProfile.updateOne({ userId: new Types.ObjectId(userId) }, { $unset: { certifications: 1 } }, { session });
        // Add new certifications if provided
        if (certifications.length > 0) {
            await UserProfile.updateOne({ userId: new Types.ObjectId(userId) }, {
                $push: {
                    certifications: {
                        $each: certifications.map((cert) => ({
                            ...cert,
                            name: cert.name?.trim(),
    ...
    async searchProfiles(searchParams) {
        try {
            const { query, field, experienceLevel, location, skills, targetLanguage, limit = 20, skip = 0 } = searchParams;
            // Create cache key
            ...
            if (location)
                dbQuery.location = new RegExp(location, 'i');
            if (targetLanguage)
                dbQuery.targetLanguage = new RegExp(targetLanguage, 'i');
            ...
            const profiles = profilesResult.map(profile => profile.toObject());
            const result = {
                profiles,
                total,
     ...
     */
    async getProfilesByRole(role, options = {}) {
        try {
            // Validate that the role is a valid UserRole
            const validRoles = ['student', 'teacher', 'professional', 'admin', 'content-creator'];
            ...
            const plainProfiles = profiles.map((profile) => profile.toObject());
            // Cache for 3 minutes
     ...
     */
    async getProfilesByField(field, options = {}) {
        try {

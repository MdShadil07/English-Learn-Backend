...
export class AvatarUploadService {
    ...
    OPTIMAL_SIZE = 512; // Optimal avatar size
    constructor() {
        ...
        // Ensure bucket exists on initialization (non-blocking)
        this.ensureBucketExists().catch((error) => {
            console.warn('⚠️ Bucket validation failed on startup, avatar uploads may not work:', error.message);
        });
    }
    /**
    ...
    async uploadAvatar(userId, file) {
        ...
        try {
            ...
            const uploadResult = await this.performAtomicUpload(userId, optimizedBuffer, fileName, file.mimetype, oldAvatarKey);
     ...
     */
    async performAtomicUpload(userId, buffer, fileName, mimeType, oldAvatarKey) {
        ...
        try {
            ...
            const updateResult = await profileService.updateProfile(userId, { avatar_url: uploadResult.publicUrl }, userId);
            if (!updateResult) {
     ...
     */
    async cleanupOldAvatar(userId, oldKey, currentKey) {
        try {
     ...
     */
    async batchUploadAvatars(uploads) {
        const results = await Promise.allSettled(uploads.map(async ({ userId, file }) => {
            try {
...
//# sourceMappingURL=avatarUploadService.js.map
...
class CloudStorageService {
    ...
    loadConfig() {
        return {
            provider: process.env.CLOUD_STORAGE_PROVIDER || 'supabase',
     ...
     */
    async cleanupOldFiles(userId, oldKey, folder = 'avatars', currentKey) {
        try {
     ...
     */
    async cleanupOldAvatars(userId, currentKey) {
        try {
            ...
            console.log(`📄 Found ${userFiles.length} files in ${userFolderPath}: ${userFiles.map((f) => f.name).join(', ')}`);
     ...
     */
    async cleanupOldFilesGeneric(userId, currentKey, folder) {
        try {

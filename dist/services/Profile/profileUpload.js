...
export class ProfileUploadService {
    ...
    async uploadAvatar(req, res) {
        ...
        try {
            ...
            const updateResult = await profileService.updateProfile(userId, { avatar_url: avatarUrl }, userId);
            if (!updateResult) {
    ...
    async uploadDocument(req, res) {
        try {
            ...
            const updateResult = await profileService.updateProfile(userId, updateData, userId);
            res.json({
                success: true,
    ...
    async deleteFile(req, res) {
        try {
            ...
            if (fileType === 'avatar' && profile.avatar_url) {
                ...
                const document = profile.documents?.find((doc) => doc.url?.includes(fileKey));
                if (document) {
            ...
            if (fileType === 'avatar') {
                ...
                updateData = {
                    documents: profile.documents?.filter((doc) => !doc.url?.includes(fileKey))
                };
            }
     ...
     * Background cleanup of old avatar (non-blocking)
     */
    async cleanupOldAvatar(userId, oldKey) {
        try {

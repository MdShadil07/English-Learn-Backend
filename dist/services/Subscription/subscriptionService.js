...
export class SubscriptionService {
     ...
     */
    async activateSubscription(userId, planId, paymentMethod, transactionId) {
        try {
            // Fetch the plan
     ...
     */
    async getActiveTierForUser(userId) {
        try {
     ...
     */
    async activateTestingSubscription(userId, tier, daysToExpire = 30) {
        try {
            ...
            return await this.activateSubscription(userId, testPlan._id, 'test', `test-${Date.now()}`);
        }
        catch (error) {

...
class AccuracyAgentService {
    ...
    async analyze(payload) {
        ...
        try {
            ...
            try {
                // 4. Message analysis
                const messageAnalysis = await this.analyzeMessage(payload.userMessage, payload.aiResponse || '', payload.userTier, payload.userLevel);
                // 5. Aggregation (if previousAggregate provided)
                const aggregated = await this.aggregateScores(payload.previousAggregate, messageAnalysis, payload.nPrev || 0);
     ...
     */
    async analyzeMessage(userMessage, aiResponse, tier, level) {
        ...
        try {
            ...
            analysisPromises.push(this.grammarAnalyzer
            ...
            if (features.vocabularyAnalysis && this.vocabularyAnalyzer) {
                ...
                if (!features.vocabularyAnalysis)
                    skippedFeatures.push('vocabulary');
            }
            // Spelling (always)
            analysisPromises.push(this.spellingAnalyzer
                ? this.spellingAnalyzer.analyze(userMessage)
                : Promise.resolve({ score: 75, feedback: '' }));
            // Fluency (always)
            analysisPromises.push(this.fluencyAnalyzer
     ...
     */
    async aggregateScores(previousAggregate, currentMessage, nPrev) {
        ...
        for (const category of categories) {
            const prevScore = previousAggregate.scores[category] || 0;
            const currScore = currentMessage.scores[category] || 0;
            merged.scores[category] = (prevScore * n + currScore) / (n + 1);
        }
        // Overall
     ...
     */
    calculateOverall(scores, weights) {
        let overall = 0;
     ...
     * 🎁 Apply leniency for short messages
     */
    applyLeniency(overall, wordCount) {
        if (wordCount >= CONFIG.LENIENCY_THRESHOLD) {
            return overall;
        }
        const missingWords = CONFIG.LENIENCY_THRESHOLD - wordCount;
        const leniencyFactor = Math.min(CONFIG.MAX_LENIENCY, missingWords * CONFIG.LENIENCY_FACTOR);
        return overall + (100 - overall) * leniencyFactor;
    }
    /**
     * 💎 Calculate XP earned
     */
    calculateXP(adjustedOverall, tier, scores) {
        // Base XP
    ...
    getWeights(level) {
        if (level === 'Beginner')
            return CATEGORY_WEIGHTS.beginner;
        if (level === 'Advanced' || level === 'Expert')
            return CATEGORY_WEIGHTS.advanced;
        return CATEGORY_WEIGHTS.default;
    }
    /**
     * 🎯 Calculate confidence score
     */
    calculateConfidence(message, aggregated) {
     ...
     */
    createErrorResponse(error, requestId, traceId, startTime) {
        return {
            status: 'error',
            requestId,
            messageAnalysis: this.createMinimalAnalysis(),
...
//# sourceMappingURL=AccuracyAgent.js.map
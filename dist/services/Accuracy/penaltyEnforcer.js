...
const BASIC_ENGLISH_WORDS = new Set([
...
const ACADEMIC_ENGLISH_WORDS = new Set([
...
const ENGLISH_VOCAB_DICTIONARY = (() => {
...
export const ensureStatistics = (stats) => {
    if (!stats) {
        return {
            wordCount: 0,
            sentenceCount: 0,
...
export const buildFallbackUnifiedResult = (tier, reason, languageContext) => ({
    overall: 15,
    adjustedOverall: 15,
    grammar: 12,
    vocabulary: 10,
...
export const enforcePenalty = (result, sourceText, languageContext) => {
    const clone = {
        ...result,
    ...
    const asciiWords = tokenizeAsciiWords(condensed);
    const normalizedDictionaryTokens = asciiWords
        .map((word) => normalizeEnglishToken(word, ENGLISH_VOCAB_DICTIONARY))
        .filter((token) => token.length > 0);
    const englishWordCount = asciiWords.length;
    const knownMatches = normalizedDictionaryTokens.filter((token) => ENGLISH_VOCAB_DICTIONARY.has(token));
    ...
    if (penaltyApplied) {
        ...
        const applyScorePenalty = (value, drop, floor = 0) => {
        ...
        if (detectedNonEnglish || nonLatinTriggered || englishWordCount === 0) {
            ...
            const baseError = Math.max(clone.statistics.errorCount, Math.max(Math.ceil(totalChars * 0.9 * severityMultiplier), englishWordCount * 4, 18));
            clone.statistics.errorCount = baseError;
            clone.statistics.criticalErrorCount = Math.max(clone.statistics.criticalErrorCount, Math.ceil(baseError * (nonLatinTriggered ? 0.75 : 0.6)));
            ...
            if (appliedPenalty > 0 || shortGibberish) {
                ...
                clone.statistics.criticalErrorCount = Math.max(clone.statistics.criticalErrorCount, Math.ceil(lexicalErrorBoost * 0.35));
                const errorsByCategory = {
                    ...clone.statistics.errorsByCategory,
        ...
        if (lexicalPenaltyApplied) {
            ...
            if (grammarFloor > 0) {
                ...
                const adjustedValue = typeof clone.adjustedOverall === 'number' && Number.isFinite(clone.adjustedOverall)
...
//# sourceMappingURL=penaltyEnforcer.js.map
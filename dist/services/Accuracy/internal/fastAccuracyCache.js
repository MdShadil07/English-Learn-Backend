...
class FastAccuracyCache {
    ...
    async initializeUser(userId) {
        ...
        const persistedLastUpdated = progress.accuracyData.cache?.lastUpdated || progress.accuracyData.lastCalculated;
     ...
     */
    async updateAccuracy(userId, newAccuracy) {
        ...
        const { cumulativeAccuracy, calculationCount, lastCalculated, } = calculateCumulativeAccuracy(currentForCalculation, newAccuracy);
        ...
        const updated = {
            ...
            syntax: this.clampAccuracy(cumulativeAccuracy.syntax ??
                this.computeRollingAverage(current.syntax, newAccuracy.syntax, current.messageCount, calculationCount)),
            coherence: this.clampAccuracy(cumulativeAccuracy.coherence ??
    ...
    computeRollingAverage(previous, incoming, previousCount, newCount) {
    ...
    startAutoSave() {
        this.saveInterval = setInterval(async () => {
            const dirtyUsers = Object.keys(this.cache).filter((userId) => this.cache[userId].isDirty);
     ...
     */
    async saveToRedis(userId, metrics) {
        if (!redisCache.isConnected())
            return;
        try {
            await redisCache.setJSON(`${this.CACHE_KEY_PREFIX}${userId}`, metrics, 3600 // 1 hour TTL
            );
        }
        catch (error) {
    ...
    async loadFromRedis(userId) {
        if (!redisCache.isConnected())
            return null;
        try {
            const cached = await redisCache.getJSON(`${this.CACHE_KEY_PREFIX}${userId}`);
            if (!cached) {
                return null;
            }
            if (cached.lastUpdated && !(cached.lastUpdated instanceof Date)) {
                const parsed = new Date(cached.lastUpdated);
    ...
    async invalidate(userId, options) {
        if (!userId) {
            return;
        }
        if (this.cache[userId]) {
...
//# sourceMappingURL=fastAccuracyCache.js.map
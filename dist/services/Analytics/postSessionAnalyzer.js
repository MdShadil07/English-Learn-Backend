...
class PostSessionAnalyzer {
     ...
     */
    calculateImprovementRate(progress, sevenDaysAgo, thirtyDaysAgo) {
        const accuracyHistory = progress.accuracyHistory || [];
        // Filter by date range
        const weeklyHistory = accuracyHistory.filter(h => new Date(h.date || h.timestamp) >= sevenDaysAgo);
        const monthlyHistory = accuracyHistory.filter(h => new Date(h.date || h.timestamp) >= thirtyDaysAgo);
     ...
     */
    calculateStreakHealth(progress) {
        ...
        let riskLevel = 'low';
        if (hoursSinceLastPractice > 20)
            riskLevel = 'high'; // 20+ hours without practice
        else if (hoursSinceLastPractice > 12)
     ...
     */
    calculateLearningVelocity(progress, sevenDaysAgo) {
        ...
        const accuracyHistory = progress.accuracyHistory || [];
        const weeklyAccuracy = accuracyHistory.filter(h => new Date(h.date || h.timestamp) >= sevenDaysAgo);
     ...
     */
    calculateConsistencyScore(progress, thirtyDaysAgo) {
     ...
     */
    generatePredictions(progress, improvementRate, streakHealth, learningVelocity) {
        ...
        if (streakHealth.riskLevel === 'high')
            survivalRate *= 0.5;
        else if (streakHealth.riskLevel === 'medium')
            survivalRate *= 0.75;
        return {
            nextLevelETA,
     ...
     */
    async storeAnalytics(userId, result) {
        try {
            // Option 1: Store as embedded field in Progress
            await Progress.findOneAndUpdate({ userId }, {
                $set: {
    ...
    async runWeeklyAnalysis() {
        try {
            ...
            const userIds = activeUsers.map((u) => u.userId.toString());
...
//# sourceMappingURL=postSessionAnalyzer.js.map
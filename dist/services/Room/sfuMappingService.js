...
class SFUMappingService {
    ...
    async assignRoomToSFUServer(roomId) {
    ...
    async getSFUServerForRoom(roomId) {
        const key = this.getRoomKey(roomId);
        const existing = await redisCache.get(key);
        if (existing) {
            return existing;
    ...
    async clearRoomSFUMapping(roomId) {
        const key = this.getRoomKey(roomId);
    ...
    async removeUserRoomMapping(userId) {
        const key = this.getUserKey(userId);
        await redisCache.del(key);
    }
}
...
//# sourceMappingURL=sfuMappingService.js.map
...
export class EmailVerificationService {
     ...
     */
    async sendGoogleLinkingVerification(userId, googleToken) {
        try {
     ...
     */
    async verifyAndLinkGoogleAccount(code) {
        try {
            ...
            const result = await googleOAuthService.linkGoogleAccount(verificationData.userId, verificationData.googleToken);
            if (!result.success) {
     ...
     */
    async resendVerificationCode(userId, type = 'link_google') {
        try {
     ...
     */
    async sendEmailOnlyGoogleLinkingVerification(userId, email) {
        try {
            const user = await User.findById(userId);
            if (!user) {
                return { success: false, message: 'User not found' };
     ...
     */
    async verifyEmailCodeAndLinkGoogle(userId, email, code) {
        try {
            const user = await User.findById(userId);
            if (!user) {
                return { success: false, message: 'User not found' };
     ...
     */
    async resendEmailOnlyGoogleLinkingVerification(userId, email) {
        try {
            const user = await User.findById(userId);
            if (!user) {
                return { success: false, message: 'User not found' };

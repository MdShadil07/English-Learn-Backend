...
import AiChatSettings from '../../models/AiChatSettings.js';
...
class AiChatSettingsService {
     ...
     */
    async updateResponseLanguage(userId, language) {
        try {
            const userObjectId = new mongoose.Types.ObjectId(userId);
     ...
     */
    async getEffectiveLanguage(userId) {
        try {
            const userObjectId = new mongoose.Types.ObjectId(userId);
            // Get settings
            const settings = await AiChatSettings.getOrCreateSettings(userObjectId);
...
//# sourceMappingURL=aiChatSettingsService.js.map
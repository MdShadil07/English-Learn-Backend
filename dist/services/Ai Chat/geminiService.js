...
const buildAccuracyAnalysis = (result, sourceText) => {
...
export class GeminiAIService {
    ...
    setupWorkers(Worker, Job) {
        if (!this.responseQueue || !this.analysisQueue) {
            console.warn('⚠️ Workers not started - queues not available');
            return;
        }
        // Worker for AI responses
        new Worker('ai-responses', async (job) => {
            const { userMessage, personality, conversationHistory, language, userId } = job.data;
            try {
                const response = await this.generateResponseInternal(userMessage, personality, conversationHistory, language, undefined, undefined, undefined, userId);
                return response;
            }
            catch (error) {
        ...
        new Worker('accuracy-analysis', async (job) => {
            try {
                const analysis = await this.analyzeMessageInternal(job.data.message, job.data.userId, normalizeTier(job.data.userTier));
                return analysis;
            }
            catch (error) {
    ...
    buildGenerationConfigForTier(tier, fastFallback = false) {
        const tierMaxTokens = (() => {
            if (tier === 'premium') {
    ...
    logGeminiFailure(error, attempt, willRetry) {
    ...
    async generateResponseInternal(userMessage, personality, conversationHistory, language, userNativeLanguage, userTier, responseLanguage, userId) {
        ...
        try {
            ...
            const shouldAnalyzeAccuracy = conversationHistory.length > 0 &&
    ...
    buildPrompt(personality, language, history, userMessage, userNativeLanguage, userTier, responseLanguage // User's preferred response language
    ) {
        const tierGuidance = {
...
Provide a helpful, educational response ${personality.tier === 'free' ? 'in plain text only' : `using the task-specific markers shown above - NO MARKDOWN. Focus on ${taskType} learning!`}:`;
    ...
    async analyzeMessage(message, userId, tier) {
        return this.analyzeMessageInternal(message, userId, normalizeTier(tier));
    }
    async analyzeMessageInternal(message, userId, tier = 'free') {
        const trimmedMessage = message?.trim?.() ?? '';
        const languageContext = detectLanguage(trimmedMessage);
        if (!trimmedMessage) {
            return buildAccuracyAnalysis(buildFallbackUnifiedResult(tier, 'Empty messages do not earn XP. Please respond in English.', languageContext), '');
        }
        try {
            ...
            return buildAccuracyAnalysis(buildFallbackUnifiedResult(tier, 'We had trouble analyzing this response. Try replying in clear English.', languageContext), trimmedMessage);
        }
    }
    async close() {
        try {

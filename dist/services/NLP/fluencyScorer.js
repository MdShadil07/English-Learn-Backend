...
const ensureTsxExecArgs = (initialExecArgv) => {
...
const transformerWorkerPending = new Map();
const flushPendingWorkerRequests = (error) => {
...
const configureWorkerStreams = (worker) => {
...
const createTransformerWorker = () => {
    ...
    for (const spec of candidateSpecs) {
        const candidateOptions = { ...baseOptions };
...
const runTransformerInference = (text) => {
...
class FluencyScorerService {
    mapPerplexityToScore(perplexity) {
    ...
    async computeTransformerFluency(text) {
    ...
    async analyzeWithTransformer(text) {
        ...
        try {
            ...
            improvements.unshift(`AI model confidence: ${Math.round(transformerScore)} / 100 (perplexity ${Number.isFinite(perplexity) ? perplexity.toFixed(1) : '∞'}).`);
            return {
                ...ruleBased,
                score: blendedScore,
                improvements,
     ...
     */
    generateImprovements(sentenceCount, avgLength, transitions, punctScore, structScore) {
        const improvements = [];
        if (avgLength < 8) {
     ...
     * Identify strengths
     */
    identifyStrengths(sentenceCount, avgLength, transitions, punctScore, structScore, coherenceScore) {
        const strengths = [];
        if (avgLength >= 12 && avgLength <= 20) {
    ...
    async analyzeRuleBased(text) {
        try {
            ...
            const overallScore = Math.round(punctuationScore * 0.25 +
                structureScore * 0.35 +
                coherenceScore * 0.40);
            ...
            const improvements = this.generateImprovements(sentences.length, avgSentenceLength, transitions, punctuationScore, structureScore);
            const strengths = this.identifyStrengths(sentences.length, avgSentenceLength, transitions, punctuationScore, structureScore, coherenceScore);
...
//# sourceMappingURL=fluencyScorer.js.map
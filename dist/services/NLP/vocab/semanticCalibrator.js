...
const LEVEL_SIMILARITY_THRESHOLDS = {
    B1: 0.52,
    B2: 0.56,
    C1: 0.6,
    C2: 0.62,
...
const toVector = (payload) => {
    ...
    if (Array.isArray(payload)) {
        ...
        if (typeof first === 'number') {
            return normalizeVector(payload.map((value) => Number(value) || 0));
        }
    }
...
class SemanticVocabularyCalibrator {
    ...
    async ensureExtractor() {
        ...
        if (!this.extractorPromise) {
            this.extractorPromise = (async () => {
                ...
                const featureExtractor = await transformersModule.pipeline('feature-extraction', EMBEDDING_MODEL_ID, {
                    quantized: true,
                    pooling: 'mean',
                    normalize: true,
                });
    ...
    async ensureAnchorVectors() {
        ...
        if (!this.anchorVectorsPromise) {
            this.anchorVectorsPromise = (async () => {
                ...
                const anchorEntries = await Promise.all(Object.entries(EMBEDDING_ANCHORS).map(async ([level, words]) => {
                    const vectors = [];
                    ...
                    return [level, averageVectors(vectors)];
                }));
    ...
    async embedWord(word, extractor) {
...
//# sourceMappingURL=semanticCalibrator.js.map
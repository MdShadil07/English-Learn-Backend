...
export async function getOrtSession(modelPath, options) {
    if (sessionCache)
        return sessionCache;
    ...
    const dynamicImport = (m) => {
        return new Function('m', 'return import(m);')(m);
    };
    try {
    ...
    try {
        if (ort && ort.logging && typeof ort.logging.setLoggingLevel === 'function') {
            try {
                ort.logging.setLoggingLevel('warning');
            }
            catch { }
        }
        if (ort && ort.logging && typeof ort.logging.setLogLevel === 'function') {
            try {
                ort.logging.setLogLevel('warning');
            }
            catch { }
        }
    }
    ...
    sessionCache = await ort.InferenceSession.create(modelPath, createOptions);
    return sessionCache;
}
export function clearOrtSession() {
    sessionCache = null;
}
//# sourceMappingURL=ortSession.js.map
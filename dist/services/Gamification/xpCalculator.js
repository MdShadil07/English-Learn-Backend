 ...
 */
import { XP_CURVE_CONFIG, ACCURACY_MULTIPLIERS, STREAK_BONUSES, LEVEL_DIFFICULTY_MODIFIERS, XP_PENALTY_RULES, } from './config.js';
// ========================================
 ...
 */
export const calculateXPForLevel = (level, prestigeLevel = 0) => {
    if (level <= 1)
        return 0;
 ...
 */
export const calculateCumulativeXP = (level, prestigeLevel = 0) => {
    let total = 0;
 ...
 */
export const getLevelFromXP = (totalXP, prestigeLevel = 0) => {
    let level = 1;
    let cumulativeXP = 0;
 ...
 */
export const getAccuracyMultiplier = (accuracy) => {
 ...
 */
export const calculateMomentumMultiplier = (momentumSystem) => {
    if (!momentumSystem.bonusActive)
        return 1.0;
 ...
 */
export const getPrestigeMultiplier = (prestigeSystem) => {
    return prestigeSystem.prestigeXPBonus;
};
const getLevelDifficultyModifier = (currentLevel) => {
...
const calculateErrorPenalty = (baseAmount, errorCount = 0, criticalErrorCount = 0, accuracy = 100) => {
    if (baseAmount <= 0) {
        return 0;
    }
...
const calculateAccuracyBonus = (baseAmount, accuracy = 0) => {
    if (accuracy >= 99) {
 ...
 */
export const calculateAdaptiveDifficulty = (recentAccuracyTrend, consistencyScore, improvementRate) => {
    // Base multiplier
    let multiplier = 1.0;
 ...
 */
export const calculateDecay = (lastActiveDate, decayConfig) => {
 ...
 */
export const calculateTotalXP = (params) => {
    const { baseAmount, accuracy = 100, streakDays = 0, tierMultiplier = 1.0, adaptiveDifficulty, momentumSystem, prestigeSystem, eventMultiplier = 1.0, isPerfectMessage = false, } = params;
    ...
    let penaltyXP = 0;
    const pushSource = (type, amount, description, multiplier) => {
        if (amount === 0) {
            return;
        }
        sources.push({
    ...
    const applyMultiplier = (multiplier, positiveType, description) => {
    ...
    const errorPenalty = calculateErrorPenalty(effectiveBaseForPenalty, params.errorCount, params.criticalErrorCount, accuracy);
    if (errorPenalty > 0) {
        totalXP -= errorPenalty;
        pushSource('penalty', -errorPenalty, `Error penalty (${params.errorCount || 0} issues)`, 1.0);
    }
    if (accuracy < 60) {
        const lowAccuracyPenalty = Math.round(baseAmount * ((60 - accuracy) / 100) * 0.6);
    ...
    const totalMultiplier = accuracyMult *
        streakMult *
        tierMultiplier *
        adaptiveMult *
        decayMult *
 ...
 */
export const distributeSkillXP = (totalXP, skillBreakdown) => {
    if (!skillBreakdown) {
 ...
 */
export const forecastLevelUp = (currentXP, xpToNextLevel, avgXPPerDay) => {
    if (avgXPPerDay <= 0)
        return null;
 ...
 */
export const calculateAverageXPPerDay = (totalXP, activeDays) => {
    if (activeDays <= 0)
        return 0;
 ...
 */
export const calculateXPVelocity = (recentXP, windowDays = 7) => {
    if (recentXP.length < 2)
        return 0;

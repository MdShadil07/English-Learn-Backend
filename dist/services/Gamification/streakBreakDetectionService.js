...
export class StreakBreakDetectionService {
     ...
     */
    static async checkDailyGoalMet(userId) {
        try {
     ...
     */
    static async resetStreakForGoalNotMet(userId) {
        try {
            const progress = await Progress.findOne({ userId });
            if (!progress) {
                return null;
     ...
     */
    static async resetStreakForTimeExpired(userId, gracePeriodHours) {
        try {
            const progress = await Progress.findOne({ userId });
     ...
     */
    static async batchResetExpiredStreaks() {
        try {
            const now = new Date();
            ...
            for (const progress of activeUsers) {
                ...
                const tierNum = progress.tier || 1;
                const gracePeriodHours = tierNum <= 2 ? 2 : tierNum <= 4 ? 4 : 6;
                const deadline = 24 + gracePeriodHours;
                if (hoursSince > deadline) {
                    // Reset to 0
                    const result = await this.resetStreakForTimeExpired(progress.userId, gracePeriodHours);
                    if (result) {
                        reset++;
                    }
                }
     ...
     */
    static async getUsersAtRisk(hoursThreshold = 20) {
        try {
            const now = new Date();
...
//# sourceMappingURL=streakBreakDetectionService.js.map
...
export const normalizeXpSnapshot = (raw) => {
    ...
    const cumulativeCurrent = typeof raw.cumulativeXPForCurrentLevel === 'number'
        ? Math.max(0, Math.floor(raw.cumulativeXPForCurrentLevel))
        : calculateCumulativeXp(level, prestigeLevel);
    const cumulativeNext = typeof raw.cumulativeXPForNextLevel === 'number'
    ...
    const progressPercentage = typeof raw.progressPercentage === 'number'
...
export const computeLevelSnapshot = (totalXp, prestigeLevel = 0, levelHint) => normalizeXpSnapshot({
    totalXp,
    prestigeLevel,
    currentLevel: levelHint,
});
//# sourceMappingURL=xpProgress.js.map
    ...
    try {
        ...
        allKeys.forEach((key) => {
            if (key.startsWith('accuracy:'))
                categorizedKeys.accuracy++;
            else if (key.startsWith('streak:'))
                categorizedKeys.streak++;
            else if (key.startsWith('progress:'))
                categorizedKeys.progress++;
            else if (key.startsWith('leaderboard:'))
                categorizedKeys.leaderboard++;
            else if (key.startsWith('session:'))
                categorizedKeys.session++;
            else
                categorizedKeys.other++;
        });
        ...
        return res.json({
            ...
            data: {
                ...
                performance: {
                    ...
                    recommendedAction: parseFloat(hitRate) < 70 ? 'Consider increasing TTLs or cache warmup' :

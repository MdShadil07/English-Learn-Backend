...
export class FinalWeightedAccuracyService {
    redisClient = null;
    persistModulo = 5; // persist to DB every N messages to reduce writes
    constructor() {
        try {
    ...
    async calculateEnhancedWeightedAccuracy(userId, currentAccuracy, errorCount // 🚀 NEW: Error count for graduated penalties
    ) {
        ...
        const weights = this.calculateAdaptiveWeights(messageCount, historicalContext?.trend, normalizedCurrent, previousAccuracy);
        ...
        if (errorCount !== undefined && errorCount > 0) {
            const wordCount = normalizedCurrent.statistics?.wordCount || 0;
    ...
    async safeRedisSetJson(key, value, ttlSec) {
        try {
            if (!this.redisClient)
                return false;
    ...
    async getHistoricalContext(userId) {
        ...
        const cached = await this.safeRedisGetJson(key);
        if (cached) {
    ...
    async fetchPreviousAccuracy(userId) {
        try {
    ...
    async getMessageCount(userId) {
        try {
            ...
            const cached = await this.safeRedisGetJson(cacheKey);
            if (cached)
    ...
    // 🔹 Core Logic
    // ==========================
    calculateAdaptiveWeights(messageCount, trend, currentAccuracy, previousAccuracy) {
    ...
    applyWeightedCalculation(current, previous, weights) {
    ...
    applySmoothing(weighted, previous, messageCount) {
    ...
    calculateTrend(existing, newAcc) {
        if (!existing) {
    ...
    // 🔹 Context Updates
    // ==========================
    async updateHistoricalContext(userId, weighted, messageCount, trend) {
        // Try Redis first
        if (this.redisClient) {
            const context = {
                userId,
     ...
     */
    applyGraduatedPenalty(score, errorCount, wordCount) {
     ...
     */
    calculatePenaltyFactor(data) {
        // Check critical metrics for severely low performance
        const criticalMetrics = ['grammar', 'spelling', 'fluency'];
        const lowMetrics = [];
        for (const metric of criticalMetrics) {
            const value = this.getNumber(data[metric], 0, 0);
            if (value < 50) {
    ...
    getNumber(value, fallback, alt) {
        if (value === undefined || value === null)
            return Number(fallback ?? alt ?? 0);
    ...
    normalizeAccuracyInput(input) {
        const out = {};
        for (const k of CATEGORY_KEYS) {
            const key = k;
            const value = this.clamp(this.getNumber(input?.[key], 0, 0));
            out[key] = value; // Type assertion for optional fields
        }
        return out;
    }
    getDefaultAccuracy() {
...
//# sourceMappingURL=enhancedWeightedAccuracy.js.map
...
const debugConsoleWarn = (...args) => {
    if (NLP_DEBUG_LOGS_ENABLED) {
        console.warn(...args);
    }
};
const DIALECT_NORMALIZATIONS = [
...
const normalizeDialectVariants = (text) => {
    let normalized = DIALECT_NORMALIZATIONS.reduce((acc, { pattern, replacement }) => acc.replace(pattern, replacement), text);
...
export function summarizeCategoryTrend(currentScore, previousScore, sampleSize = 0) {
 ...
 */
function extractErrorsFromAIResponse(userMessage, aiResponse, result, features) {
...
export class UnifiedAccuracyCalculator {
    ...
    languageToolDetector = null;
    openRouterFluencyDetector = null;
    redisCache = null;
    constructor() {
    ...
    summarizeSnapshot(label, snapshot) {
        if (!snapshot) {
            return `${label}: n/a`;
        }
    ...
    logNLPContributions(contributions) {
        if (!contributions) {
            return;
        }
        const summary = {
    ...
    async applySpellingContributionFromNLP(result) {
        ...
        const hasValidDict = spellingContribution?.hasValidDictionary ?? true;
        if (!hasValidDict) {
            ...
            try {
                result.spelling = null;
                if (!result.categoryDetails)
                    result.categoryDetails = {};
                result.categoryDetails.spelling = result.categoryDetails.spelling || {};
                result.categoryDetails.spelling.score = null;
                result.categoryDetails.spelling.unavailable = true;
            }
            catch (e) {
        ...
        const ltDetails = result.nlpContributions?.languageTool?.details || [];
        const typoDetails = spellingContribution.details || [];
        ...
        const addToMap = (key, entry) => {
            if (!mergedMap[key])
                mergedMap[key] = { word: entry.word, start: entry.start, end: entry.end, sources: new Set(), suggestions: new Set() };
        ...
        ltTypos.forEach((d) => {
            ...
            const suggs = [];
            if (d.suggestion)
                suggs.push(d.suggestion);
            if (Array.isArray(d.alternatives))
                suggs.push(...d.alternatives.slice(0, 3));
        ...
        if (spellingMetrics) {
            ...
            if (!spellingMetrics.contentTokenCount && !spellingMetrics.functionTokenCount) {
                ...
                try {
                    const tokens = result._messageForAnalysis ? result._messageForAnalysis.split(/\s+/) : [];
                    let content = 0;
                    let func = 0;
                    tokens.forEach((tk) => {
                        if (!tk)
        ...
        if (typeof combinedErrors === 'number') {
            ...
            if (result.statistics.errorsByCategory) {
                result.statistics.errorsByCategory.spelling = Math.max(result.statistics.errorsByCategory.spelling ?? 0, combinedErrors);
            }
        }
        result.nlpContributions.spelling = {
            ...spellingContribution,
            appliedToScore: true,
    ...
    async applyLanguageToolContributionFromNLP(result) {
        ...
        try {
            const tokenLang = result.nlpContributions?.tokenLanguage;
        ...
        if (!result.statistics.errorsByCategory) {
            result.statistics.errorsByCategory = {};
        }
        ...
        result.nlpContributions.languageTool = {
            ...result.nlpContributions.languageTool,
        ...
        if (ltErrors === 0 && ltZeroStrict) {
            ...
            const spellingIssuesCount = Number(result.nlpContributions?.spelling?.errorsFound ?? result.categoryDetails?.spelling?.totalErrors ?? 0);
        ...
        try {
            ...
            for (const d of ltDetails) {
                ...
                const confidence = typeof d.confidence === 'number' ? Math.max(0.5, Math.min(1, d.confidence)) : 1;
                // Historical occurrence factor: if Redis is available, use rule frequency to slightly increase weight
                let occurrenceFactor = 1;
                try {
                    if (typeof redisCache !== 'undefined' && redisCache && redisCache.isConnected && redisCache.isConnected()) {
                        const client = redisCache.getClient();
            ...
            const criticalErrorCount = result.errors.filter((e) => e.severity === 'critical' || e.severity === 'error').length;
        ...
        try {
            ...
            if (this.redisCache) {
                try {
                    ...
                    const cachedUserData = await this.redisCache.get(cacheKey);
                    if (cachedUserData) {
            ...
            const proficiencyLevel = typeof rawLevel === 'string'
     ...
     */
    async analyzeMessage(message, aiResponse = '', options = {}) {
        const startTime = Date.now();
        let { tier = 'premium', proficiencyLevel, userId, previousAccuracy, enableNLP = this.NLP_ENABLED, enableWeightedCalculation = !!userId, redisClient, historicalWeighting, languageContext, } = options;
        ...
        if (languageContext) {
            debugConsoleLog(`🌐 Language detection → ${languageContext.primaryLanguageName} (${(languageContext.probability * 100).toFixed(1)}%), englishRatio=${languageContext.englishRatio}`);
        }
        ...
        try {
            ...
            const basicResult = await this.performBasicAnalysis(message, aiResponse, tier, level, userId, enableWeightedCalculation, undefined, // previousAccuracy intentionally omitted here
            undefined, // historicalWeighting intentionally omitted here
            languageContext, enableNLP, options.aiPersonality);
            ...
            let nlpContributions = undefined;
            if (enableNLP) {
                ...
                try {
                    ...
                    if (fallback.errors.length > 0) {
                        fallback.errors.forEach((fe) => {
                            ...
                            if (!exists)
                                basicResult.errors.push(fe);
                        });
                    }
                    const ltCount = Number(basicResult.nlpContributions?.languageTool?.errors || 0);
                ...
                try {
                    const contrib = basicResult.nlpContributions || {};
                    ...
                    try {
                        ...
                        if (!Number.isFinite(totalErrors) || Number.isNaN(totalErrors))
                            totalErrors = 0;
                        // critical errors from LanguageTool details (if available)
                        const ltDetails = basicResult.nlpContributions?.languageTool?.details || [];
                        const criticalErrors = ltDetails.filter((d) => d && (d.severity === 'critical' || d.severity === 'error')).length || 0;
                        basicResult.statistics.totalErrors = totalErrors;
                        basicResult.statistics.criticalErrors = criticalErrors;
                        basicResult.statistics.errors = totalErrors; // Ensure errors field is set
                    }
                    catch (e) {
                        debugConsoleWarn('Failed to compute total/critical statistics', e);
                        basicResult.statistics.totalErrors = 0;
                        basicResult.statistics.criticalErrors = 0;
                        basicResult.statistics.errors = 0;
                    }
                }
                catch (statErr) {
                ...
                try {
                    const vocabContrib = basicResult.nlpContributions?.vocabulary;
                    if (vocabContrib && typeof vocabContrib.score === 'number') {
                        ...
                        basicResult.nlpContributions.vocabulary = {
                            ...basicResult.nlpContributions.vocabulary,
                            appliedToScore: true,
                        };
                ...
                try {
                    const fluencyContrib = basicResult.nlpContributions?.fluency;
                    if (fluencyContrib && typeof fluencyContrib.score === 'number') {
                        ...
                        basicResult.nlpContributions.fluency = {
                            ...basicResult.nlpContributions.fluency,
                            appliedToScore: true,
                        };
            ...
            if (false && !alreadyWeighted && enableWeightedCalculation && userId) {
                try {
                    ...
                    const enhancedResult = await enhancedWeightedAccuracyService.calculateEnhancedWeightedAccuracy(userId, {
                    ...
                    const hw = historicalWeighting;
                    const perf = basicResult.performance;
                    if (hw && perf) {
                        ...
                        if (hw.categoryBaselines) {
                            const baselineKeys = Object.keys(hw.categoryBaselines)
                                .filter((key) => NUMERIC_ACCURACY_KEYS.includes(key));
                    ...
                    debugConsoleLog('🪄 [UnifiedAccuracy] Weighted snapshots', `${this.summarizeSnapshot('current', basicResult.currentAccuracy)} | ${this.summarizeSnapshot('weighted', basicResult.weightedAccuracy)}`);
                    ...
                    debugConsoleWarn('  Reason:', error.message || String(error));
            ...
            debugConsoleLog('🎯 [UnifiedAccuracy] Final snapshots', `${this.summarizeSnapshot('current', basicResult.currentAccuracy)} | ${this.summarizeSnapshot('weighted', basicResult.weightedAccuracy)} | ${this.summarizeSnapshot('baseline', baselineSnapshot)}`);
            if (basicResult.categoryDetails) {
     ...
     */
    async performBasicAnalysis(message, aiResponse, tier, level, userId, enableWeightedCalculation, previousAccuracy, historicalWeighting, languageContext, enableNLP, aiPersonality) {
        const features = TIER_FEATURES[tier];
        // Initialize result
        const result = {
            overall: 100,
        ...
        if (features.basicVocabulary && !enableNLP) {
            try {
                ...
                result.vocabularyLevel = vocabAnalysis.level;
            }
            catch (err) {
        ...
        const nlpFluencyApplied = result.nlpContributions?.fluency?.appliedToScore;
        ...
        try {
            const hiRatio = (result.nlpContributions?.tokenLanguage?.msgLangProportions?.hi) ?? (result.languageContext?.hindiRatio ?? 0);
            if (hiRatio > 0.9) {
                result.punctuation = 100;
                // Remove punctuation errors added earlier
                result.errors = result.errors.filter((e) => e.type !== 'punctuation');
            }
        }
        catch (e) {
            // ignore
        ...
        try {
            ...
            if (hiRatio > 0.9) {
                ...
                result.errors = result.errors.filter((e) => e.type !== 'capitalization');
            }
        }
        catch (e) {
            // ignore
        ...
        result.statistics = {
            ...
            criticalErrorCount: Number(result.errors?.filter((e) => e?.severity === 'critical').length || 0),
            errors: Number(result.errors?.length || 0),
            totalErrors: Number(result.errors?.length || 0),
            criticalErrors: Number(result.errors?.filter((e) => e?.severity === 'critical').length || 0),
        ...
        if (false && userId && enableWeightedCalculation) {
            try {
                ...
                debugConsoleLog('🪄 [UnifiedAccuracy] Weighted snapshots', `${this.summarizeSnapshot('current', result.currentAccuracy)} | ${this.summarizeSnapshot('weighted', result.weightedAccuracy)}`);
                result.performance = {
     ...
     */
    async performNLPAnalysis(message, redisClient, languageContext, tier = 'free', level) {
        ...
        try {
            ...
            await spellingChecker.initialize();
            ...
            if (languageContext?.shouldSkipEnglishChecks || (languageContext?.hindiRatio ?? 0) > 0.4) {
                ...
                const spellingReport = await spellingChecker.getReport(normalizedForSpelling);
                contributions.spelling = {
        ...
        try {
            ...
            if (tier === 'premium' && OPENROUTER_ENABLED && this.openRouterFluencyDetector) {
                ...
                if (this.redisCache && NLP_CACHE_ENABLED) {
                    const cached = await this.redisCache.get(`fluency:${message}`);
                    if (cached) {
     ...
     */
    runFallbackGrammarChecks(message) {
        const errors = [];
        const src = message || '';
        const lower = src.toLowerCase();
        const add = (msg, start, end, suggestion, severity = 'major', rule) => {
            errors.push({
                type: 'grammar',
                category: 'fallback',
                severity,
     ...
     */
    async analyzeGrammar(message, tier, level, languageContext, enableNLP) {
        const errors = [];
        const feedback = [];
        ...
        if (!enableNLP && LANGUAGETOOL_ENABLED && this.languageToolDetector) {
            try {
                ...
                ltErrors.forEach((ltError) => {
                    ...
                    let severity = ltError.severity || (ltError.type === 'grammar' ? 'critical' : 'medium');
                    ...
                    if (isGrammarCategory) {
                        addError({
                            type: 'grammar',
                            category: ltError.category || 'correctness',
                            severity,
                        ...
                        const allowedTypes = new Set(['grammar', 'vocabulary', 'spelling', 'fluency', 'punctuation', 'capitalization', 'syntax', 'coherence', 'style', 'idiom', 'collocation', 'semantic']);
                        addError({
                            type: allowedTypes.has(String(ltError.type)) ? ltError.type : 'grammar',
                            category: ltError.category || 'other',
                            severity,
                            message: ltError.message,
                            position: { start: resolvedStart, end: resolvedEnd, word: ltError.position?.word },
                            suggestion: ltError.suggestion,
        ...
        const grammarRelevantTypes = ['grammar', 'syntax', 'semantic', 'style'];
        const stylePenaltySeverities = ['medium', 'high', 'major', 'critical'];
        ...
        if (finalTotalErrors === 0) {
            ...
            dominantIssues.push(...Array.from(groupedByRule.values())
     ...
     */
    analyzeTone(message, aiPersonality) {
        ...
        let passiveCount = 0;
        words.forEach(w => {
            if (formalWords.has(w))
                formalCount++;
            if (informalWords.has(w))
                informalCount++;
            if (assertiveWords.has(w))
                assertiveCount++;
            if (passiveWords.has(w))
                passiveCount++;
        });
        ...
        if (formalityScore >= 75)
            tone = 'formal';
        else if (formalityScore <= 25)
            tone = 'casual';
        else if (formalityScore <= 45)
            tone = 'informal';
     ...
     */
    calculateReadability(message, statistics) {
        ...
        if (gradeLevel >= 16)
            averageLevel = 'Graduate';
        else if (gradeLevel >= 13)
            averageLevel = 'College';
        else if (gradeLevel >= 9)
            averageLevel = 'High School';
        else if (gradeLevel >= 6)
            averageLevel = 'Middle School';
        return {
    ...
    analyzeCoherence(message) {
        ...
        let transitionTypes = new Set();
        words.forEach(w => {
            if (additive.has(w)) {
                transitionCount++;
                transitionTypes.add('additive');
            }
            if (adversative.has(w)) {
                transitionCount++;
                transitionTypes.add('adversative');
            }
            if (causal.has(w)) {
                transitionCount++;
                transitionTypes.add('causal');
            }
            if (sequential.has(w)) {
                transitionCount++;
                transitionTypes.add('sequential');
            }
        });
     ...
     */
    analyzeStyle(message) {
        const sentences = message.split(/[.!?]+/).filter(s => s.trim().length > 0);
        const words = message.toLowerCase().split(/[\s,.-]+/).filter(w => w.length > 0);
        if (sentences.length === 0)
            return { passiveVoiceUsage: 0, sentenceVariety: 100, recommendations: [] };
        ...
        if (stdDev < 1.5)
            sentenceVariety = 40; // Too robotic, identical lengths
        else if (stdDev < 3)
            sentenceVariety = 70; // Okay but could be better
        else if (stdDev > 15)
            sentenceVariety = 60; // Erratically long/short
        const recommendations = [];
        ...
        const starterCounts = starters.reduce((acc, s) => { acc[s] = (acc[s] || 0) + 1; return acc; }, {});
        const repetitive = Object.entries(starterCounts).filter(([_, count]) => count > 2);
     ...
     */
    analyzeAIResponse(userMessage, aiResponse, errors, features) {
        if (!aiResponse) {
            return {
                hasCorrectionFeedback: false,
                hasGrammarCorrection: false,
        ...
        const hasCorrectionMarker = aiResponse.includes('[ERROR:') ||
            aiResponse.includes('[CORRECTION') ||
            aiResponse.includes('✓') ||
            aiResponse.includes('[NOTE');
        const hasCorrectionFeedback = hasCorrectionMarker ||
            /should be|correct|mistake|error|wrong|incorrect|fix|change|instead of|better to say/i.test(aiResponse);
        const hasGrammarCorrection = /grammar|word order|verb|tense|structure|subject-verb|question formation|pronoun/i.test(aiResponse);
        ...
        const engagementScore = Math.min(100, (hasCorrectionFeedback ? 30 : 0) +
     ...
     */
    analyzePremiumFeatures(message, tier, errors) {
        const features = TIER_FEATURES[tier];
            ...
            while ((match = verbPattern.exec(message)) !== null) {
                ...
                if (noun) {
                    ...
                    if (!isCorrect && noun === 'mistake' && verb === 'do') {
                        ...
                        if (Math.random() > 0.5) {
                            collocationIssues.push({ phrase: match[0], correctAlternatives: ['take a break'], explanation: `"Take a break" is often preferred in American English.` });
                        }
                    }
                }
            }
        ...
        const advancedVocabSuggestions = [];
        ...
        if (tier === 'premium' || tier === 'pro') {
            ...
            const starters = sentences.map(s => s.trim().split(/\s+/)[0]?.toLowerCase()).filter(Boolean);
            const starterCounts = starters.reduce((acc, s) => { acc[s] = (acc[s] || 0) + 1; return acc; }, {});
            const repetitive = Object.entries(starterCounts).filter(([_, count]) => count > 1);
            if (repetitive.length > 0) {
     ...
     */
    generateShorterVersion(sentence) {
    ...
    convertToActiveVoice(sentence) {
        ...
        const passivePatterns = [
            { passive: /is (being )?(\w+ed) by/, active: (match) => match.replace(/is (being )?(\w+ed) by/, '') },
            { passive: /was (being )?(\w+ed) by/, active: (match) => match.replace(/was (being )?(\w+ed) by/, '') },
            { passive: /were (being )?(\w+ed) by/, active: (match) => match.replace(/were (being )?(\w+ed) by/, '') },
        ];
     ...
     */
    generateAlternativePhrasings(phrase) {
        ...
        if (alternatives.length === 0) {
            alternatives.push('🎯 Try rephrasing for clarity', '✨ Consider a more formal expression', '💡 Use more specific vocabulary');
        }
     ...
     */
    getAnalysisDepth(tier) {
        return TIER_FEATURES[tier].analysisDepth;
    }
    determineProficiencyLevel(message) {
        const words = message.split(/\s+/).length;
        const avgWordLength = message.split(/\s+/).reduce((sum, word) => sum + word.length, 0) / Math.max(words, 1);
        if (words < 10 || avgWordLength < 4)
            return 'Beginner';
        if (words < 25 || avgWordLength < 5)
            return 'Intermediate';
        if (words < 50 || avgWordLength < 6)
            return 'Advanced';
        return 'Expert';
    }
    calculateOverallScore(result) {
        ...
        if (criticalErrorCount > 10 && categoryWeights.grammar > 0.25) {
            ...
            const redistributionKeys = ['vocabulary', 'spelling', 'fluency', 'punctuation', 'capitalization'];
        ...
        const weightedScore = (result.grammar || 0) * categoryWeights.grammar +
    ...
    captureAccuracySnapshot(result) {
        ...
        const snapshot = {
            ...
            vocabularyLevel: result.vocabularyLevel || result.vocabularyAnalysis?.level,
    ...
    cloneAccuracySnapshot(snapshot) {
        if (!snapshot) {
            return {};
        }
    ...
    applyHistoricalSmoothingFallback(result, tier, previousAccuracy, baselineCurrent, weightingConfig) {
        ...
        if (weightingConfig && weightingConfig.disableHistorical) {
        ...
        NUMERIC_ACCURACY_KEYS.forEach((key) => {
            const currentValue = this.clampScore(currentSnapshot[key]);
            const previousValue = this.clampScore(previousAccuracy[key]);
            ...
            const baseline = categoryBaselines && Object.prototype.hasOwnProperty.call(categoryBaselines, key)
                ...
                : undefined;
            const fallbackPrev = previousValue ?? baseline ?? currentValue ?? 0;
            ...
            weightedSnapshot[key] = smoothedValue;
        });
        ...
        result.performance = {
            ...
            baselinesApplied: baselinesApplied.length > 0 ? Array.from(new Set(baselinesApplied)) : undefined,
        };
        debugConsoleLog('ℹ️ Fallback historical smoothing applied (enhanced weighting unavailable).');
    }
    attachCategoryTrends(result, previousAccuracy) {
     ...
     */
    analyzeFluency(message, tier, level) {
        const feedback = [];
        const errors = [];
        ...
        const fleschReadingEase = Math.max(0, Math.min(100, 206.835 - (1.015 * avgSentenceLength) - (84.6 * avgSyllablesPerWord)));
        ...
        sentences.forEach((s) => {
            ...
            if (wordsInSentence > 10 && commaCount === 0)
                missingCommaCount++;
        });
        ...
        if (totalFillerInstances > 0) {
            const fillerHighlights = new Set();
 ...
 */
export async function analyzeMessage(message, aiResponse = '', options = {}) {
    return await unifiedAccuracyCalculator.analyzeMessage(message, aiResponse, options);
}
// Legacy exports for backward compatibility
export async function analyzeMessageEnhanced(message, aiResponse, tier, previousAccuracy, proficiencyLevel) {
...
export async function analyzeMessageWithNLP(userMessage, aiResponse, topic, options = {}) {
...
//# sourceMappingURL=unifiedAccuracyCalculators.js.map
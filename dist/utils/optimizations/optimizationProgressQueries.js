...
const resolveLeaderboardSortField = (metric, timeframe) => {
    if (metric === 'xp') {
        if (timeframe === 'week') {
            return 'weeklyXP';
        }
...
export class OptimizedProgressQueries {
     ...
     */
    static async getLeaderboard(limit = 100, offset = 0, options = {}) {
     ...
     */
    static async getUsersWithExpiringStreaks(hoursThreshold = 4) {
        try {
            const now = new Date();
    ...
    static async batchUpdateProgress(updates) {
        try {
            ...
            await Promise.all(updates.map(({ userId }) => redisCache.del(`progress:${userId}`, `streak:${userId}`)));
            return result;
        }
        catch (error) {
    ...
    static async invalidateUserCache(userId) {
        ...
        try {
            await redisCache.del(`progress:${userIdStr}`, `streak:${userIdStr}`);
...
//# sourceMappingURL=optimizationProgressQueries.js.map
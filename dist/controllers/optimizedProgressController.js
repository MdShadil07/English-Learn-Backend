...
const buildXpSnapshot = (totalXP, currentLevel, prestigeLevel, overrides = {}) => {
    ...
    const cumulativeCurrent = typeof overrides.cumulativeXPForCurrentLevel === 'number'
        ? overrides.cumulativeXPForCurrentLevel
        : calculateCumulativeXP(safeLevel, safePrestige);
    const cumulativeNext = typeof overrides.cumulativeXPForNextLevel === 'number'
    ...
    const normalizedCurrentLevelXP = Math.max(0, Math.min(typeof overrides.currentLevelXP === 'number'
        ? overrides.currentLevelXP
        : safeTotalXP - cumulativeCurrent, levelSpan));
    const normalizedXpToNext = Math.max(0, Math.min(typeof overrides.xpToNextLevel === 'number'
        ? overrides.xpToNextLevel
        : cumulativeNext - safeTotalXP, levelSpan));
    const progressPercentage = Math.max(0, Math.min(100, typeof overrides.progressPercentage === 'number'
...
const toOptionalNumber = (value) => typeof value === 'number' && !Number.isNaN(value) ? value : undefined;
function buildAccuracyResponse(fastCachedAccuracy, optimizedCachedAccuracy, fallbackAccuracy, fallbackSource = 'progress-cache', fallbackTimestamp) {
    if (fastCachedAccuracy) {
        return {
            ...
            lastUpdated: toIsoString(fastCachedAccuracy.lastUpdated),
            source: 'fast-cache',
    ...
    if (fallbackAccuracy) {
        ...
        const messageCount = toOptionalNumber(fallbackAccuracy.messageCount) ?? toOptionalNumber(fallbackAccuracy.calculationCount) ?? 0;
        const timestampCandidate = fallbackTimestamp ??
...
const selectRealtimeFields = `
  ...
  updatedAt
`;
export const getRealtimeProgress = async (req, res) => {
    try {
        ...
        const accuracyFromCache = buildAccuracyResponse(fastCachedAccuracy, optimizedCachedAccuracy, cachedProgress?.accuracy ?? null, 'progress-cache', typeof cachedProgress?.lastUpdate === 'string' ? cachedProgress.lastUpdate : undefined);
        const totalXPFromCache = typeof cachedProgress?.xp?.total === 'number'
            ? cachedProgress.xp.total
            : typeof cachedProgress?.totalXP === 'number'
                ? cachedProgress.totalXP
                : undefined;
        const currentLevelFromCache = typeof cachedProgress?.xp?.currentLevel === 'number'
            ? cachedProgress.xp.currentLevel
            : typeof cachedProgress?.currentLevel === 'number'
                ? cachedProgress.currentLevel
                : undefined;
        const prestigeFromCache = typeof cachedProgress?.xp?.prestigeLevel === 'number'
            ? cachedProgress.xp.prestigeLevel
            : typeof cachedProgress?.prestigeLevel === 'number'
                ? cachedProgress.prestigeLevel
                : undefined;
        const hasProgressCache = typeof totalXPFromCache === 'number' && typeof currentLevelFromCache === 'number' && accuracyFromCache;
        if (hasProgressCache && accuracyFromCache) {
            const xpSnapshot = buildXpSnapshot(totalXPFromCache, currentLevelFromCache, prestigeFromCache, {
                currentLevelXP: typeof cachedProgress?.xp?.currentLevelXP === 'number'
                    ? cachedProgress.xp.currentLevelXP
                    : typeof cachedProgress?.currentLevelXP === 'number'
                        ? cachedProgress.currentLevelXP
                        : undefined,
                xpToNextLevel: typeof cachedProgress?.xp?.xpToNextLevel === 'number'
                    ? cachedProgress.xp.xpToNextLevel
                    : typeof cachedProgress?.xpToNextLevel === 'number'
                        ? cachedProgress.xpToNextLevel
                        : undefined,
                progressPercentage: typeof cachedProgress?.xp?.progressPercentage === 'number'
                    ? cachedProgress.xp.progressPercentage
                    : typeof cachedProgress?.levelProgress === 'number'
                        ? cachedProgress.levelProgress
                        : undefined,
                cumulativeXPForCurrentLevel: typeof cachedProgress?.xp?.cumulativeXPForCurrentLevel === 'number'
                    ? cachedProgress.xp.cumulativeXPForCurrentLevel
                    : undefined,
                cumulativeXPForNextLevel: typeof cachedProgress?.xp?.cumulativeXPForNextLevel === 'number'
                    ? cachedProgress.xp.cumulativeXPForNextLevel
                    : undefined,
            });
            const responseData = {
                streak: {
                    current: typeof cachedProgress?.streak?.current === 'number'
                ...
                stats: {
                    totalMessages: typeof cachedProgress?.stats?.totalMessages === 'number'
                        ...
                        : typeof cachedProgress?.stats?.conversationsPracticed === 'number'
                            ...
                            : typeof cachedProgress?.streak?.messages === 'number'
                                ...
                                : 0,
                    totalMinutes: typeof cachedProgress?.stats?.totalMinutes === 'number'
                        ? cachedProgress.stats.totalMinutes
                        : 0,
                },
                lastUpdate: typeof cachedProgress?.lastUpdate === 'string'
        ...
        const progressAccuracyFallback = progressRecord.accuracyData
            ...
            : null;
        const accuracyFromDatabase = buildAccuracyResponse(fastCachedAccuracy, optimizedCachedAccuracy, progressAccuracyFallback, 'database') ?? {
            overall: progressRecord.accuracyData?.overall ?? 0,
            adjustedOverall: progressRecord.accuracyData?.adjustedOverall ?? progressRecord.accuracyData?.overall ?? 0,
            ...
            lastUpdated: toIsoString(progressRecord.accuracyData?.lastCalculated),
            source: 'database',
        };
        ...
        const responseData = {
            ...
            stats: {
                totalMessages: typeof progressRecord.stats?.conversationsPracticed === 'number'
                    ? progressRecord.stats.conversationsPracticed
                    : typeof progressRecord.streak?.messages === 'number'
                        ? progressRecord.streak.messages
                        : 0,
                totalMinutes: typeof progressRecord.stats?.totalTimeSpent === 'number' ? progressRecord.stats.totalTimeSpent : 0,
            },
            lastUpdate: toIsoString(progressRecord.updatedAt),
        };
        try {
...
export const getOptimizedDashboard = async (req, res) => {
    try {
        const userId = req.user?._id?.toString();
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
  ...
  accuracyData.overallAccuracySummary
      ...
      `)
        ...
        const progressRecord = progress;
        const today = (progressRecord.streak?.todayProgress ?? {});
        ...
        const dashboardData = {
            ...
            todayProgress: {
                ...
                percentComplete: Math.min(100, (Number(today.minutesPracticed ?? 0) / todayMinutesGoal) * 50 +
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                status: stats.queueSize < 100 ? 'healthy' : stats.queueSize < 500 ? 'moderate' : 'high',
            },
        });
    }
    catch (error) {
...
export const forceFlushQueue = async (req, res) => {
    try {
...
//# sourceMappingURL=optimizedProgressController.js.map
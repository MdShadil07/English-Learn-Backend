 ...
 */
router.get('/fast/init', authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/fast/current', authenticate, async (req, res) => {
    try {
        const userId = req.user?._id?.toString();
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
 ...
 */
router.post('/fast/save', authenticate, async (req, res) => {
    try {
        const userId = req.user?._id?.toString();
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
 ...
 */
router.post('/fast/cleanup', authenticate, async (req, res) => {
    try {
        const userId = req.user?._id?.toString();
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
...
//# sourceMappingURL=fastAccuracyController.js.map
...
import { calculateTotalXP, getLevelInfo, calculateXPForLevel, calculateXPForNextLevel, calculateLevelFromXP, calculateCurrentLevelXP, calculateXPToNextLevel, calculateTotalXPForLevel, } from '../../services/Gamification/index.js';
/**
 * Progress Controller
...
export class ProgressController {
     ...
     */
    async calculateXPReward(req, res) {
        try {
            const { baseAmount = 10, accuracy = 100, streakDays = 0, isPremium = false, eventMultiplier = 1.0, isPerfectMessage = false, adaptiveMultiplier = 1.0, } = req.body;
            const xpParams = {
                baseAmount,
                accuracy,
                streakDays,
     ...
     */
    async calculateXPForLevel(req, res) {
        try {
     ...
     */
    async calculateXPForNextLevel(req, res) {
        try {
     ...
     */
    async calculateLevelFromXP(req, res) {
        try {
            const { totalXP } = req.body;
     ...
     */
    async calculateCurrentLevelXP(req, res) {
        try {
     ...
     */
    async calculateXPToNextLevel(req, res) {
        try {
            const { totalXP, currentLevel } = req.body;
     ...
     */
    async calculateTotalXPForLevel(req, res) {
        try {
     ...
     */
    async initializeUserLevel(req, res) {
        try {
    ...
    async updateSkills(req, res) {
        try {
            ...
            if (skills.accuracy !== undefined)
                userLevel.accuracy = skills.accuracy;
            if (skills.vocabulary !== undefined)
                userLevel.vocabulary = skills.vocabulary;
            if (skills.grammar !== undefined)
                userLevel.grammar = skills.grammar;
            if (skills.pronunciation !== undefined)
                userLevel.pronunciation = skills.pronunciation;
            if (skills.fluency !== undefined)
                userLevel.fluency = skills.fluency;
            userLevel.lastActive = new Date();
            await userLevel.save();
            res.json({
                success: true,
    ...
    async getStats(req, res) {
        try {
            ...
            const stats = {
                ...
                streak: userLevel.streak,
                totalSessions: userLevel.totalSessions,
                averageSkill: Math.round((userLevel.accuracy + userLevel.vocabulary + userLevel.grammar +

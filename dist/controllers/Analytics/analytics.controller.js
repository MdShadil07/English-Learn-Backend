...
export class AnalyticsController {
    ...
    async getDashboardAnalytics(req, res) {
        try {
            ...
            const analytics = await progressOptimizationService.getAnalyticsData(userId, timeRange);
            if (!analytics) {
                res.status(404).json({
                    success: false,
            ...
            const progress = await progressOptimizationService.getProgressData(userId, { forceRefresh: forceRefresh === 'true' });
            if (!progress) {
                res.status(404).json({
                    success: false,
            ...
            // Get XP breakdown
            const xpBreakdown = this.getXPBreakdownData(progress, timeRange);
            // Get skills overview
    ...
    async getLeaderboard(req, res) {
        try {
            const { limit = '10', offset = '0', metric = 'xp', direction = 'desc', tier, timeframe = 'all', } = req.query;
            ...
            const normalizedMetric = (metricMapping[metricKey] || 'xp');
            const normalizedDirection = direction === 'asc' ? 'asc' : 'desc';
            const timeframeKey = typeof timeframe === 'string' ? timeframe.toLowerCase() : 'all';
            const normalizedTimeframe = timeframeKey === 'week' || timeframeKey === 'month' ? timeframeKey : 'all';
            const normalizedTier = typeof tier === 'string' && tier.toLowerCase() !== 'all' ? tier.toLowerCase() : undefined;
            const leaderboard = await OptimizedProgressQueries.getLeaderboard(parsedLimit, parsedOffset, {
     ...
     */
    async updateAccuracyData(req, res) {
        try {
            const { userId } = req.params;
    ...
    // HELPER METHODS
    // ========================================
    async getAccuracyTrendsData(progress, days) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        const recentHistory = progress.accuracyHistory.filter(entry => new Date(entry.date) >= cutoffDate);
    ...
    async getLevelUpStatsData(progress) {
    ...
    getXPBreakdownData(progress, timeRange) {
    ...
    getSkillsOverviewData(progress) {
    ...
    getCategoryPerformanceData(progress) {

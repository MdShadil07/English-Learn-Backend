...
class AiChatSettingsController {
    ...
    async updateLanguage(req, res) {
        try {
            ...
            const settings = await aiChatSettingsService.updateResponseLanguage(userId.toString(), language);
            res.status(200).json({
                success: true,
    ...
    async updateSettings(req, res) {
        try {
            const userId = req.user?._id || req.user?.id;
            const { responseLanguage, useNativeLanguageForTranslations, autoDetectLanguage, alwaysShowEnglish } = req.body;
            if (!userId) {
                res.status(401).json({
                    success: false,
                    message: 'Unauthorized'
     ...
     */
    async getEffectiveLanguage(req, res) {
        try {
            const userId = req.user?._id || req.user?.id;
            if (!userId) {
                res.status(401).json({
...
//# sourceMappingURL=aiChatSettingsController.js.map
...
import { GeminiAIService } from '../../services/Ai Chat/geminiService.js';
...
router.post('/analyze', authenticate, rateLimit({
    windowMs: 60 * 1000,
    max: 30,
    ...
    try {
        ...
        try {
            ...
            const errorCount = analysisStats.errorCount ?? analysisStats.totalErrors ?? analysis?.totalErrors ?? 0;
            ...
            if (xpResult.totalXP !== 0) {
                ...
                await progressOptimizationService.addXP(userId, xpResult.totalXP, xpSource, 'conversation');
            }
...
router.get('/personalities', cache('personalities', 3600), // Cache for 1 hour
async (req, res) => {
    try {
        const user = req.user;
...
async function getAvailablePersonalities(userTier) {
    ...
    return allPersonalities.filter(p => tierOrder[p.tier] <= userTierLevel);
}

...
async function resolveBuyerState(req, userId) {
    if (req.body && req.body.buyerState)
        return req.body.buyerState;
    if (!userId)
        return null;
...
function computeGstBreakdown(amount, state, ratePercent) {
...
async function provisionOrder(payment, userId) {
...
export async function createSubscriptionProductionHandler(req, res) {
    try {
        ...
        if (!customerId) {
            try {
                const customer = await razorpaySub.createCustomer({
                    ...
                    email: user?.email,
                    contact: user.phone // Adjust if phone exists
                });
...
export async function confirmPaymentHandler(req, res) {
    try {
        ...
        await Payment.create({
            userId: req.user?._id, // Might be null if webhook, but this is confirm handler so user is likely logged in
...
export async function subscriptionSseHandler(req, res) {
    try {
...
export async function taxPreviewHandler(req, res) {
    try {
        ...
        console.error('taxPreviewHandler error', err);
        return res.status(500).json({ success: false, message: 'Failed to compute tax' });
    }
}
...
export async function getMySubscriptionHandler(req, res) {
    try {
        const userId = req.user?._id;
        if (!userId)
...
export async function getPaymentHandler(req, res) {
    try {
        ...
        console.error('getPaymentHandler error', err);

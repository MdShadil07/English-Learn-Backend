 ...
 */
export const getAdvancedStreakStatus = async (req, res) => {
    try {
 ...
 */
export const trackStreakActivity = async (req, res) => {
    try {
        const userId = req.user?._id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
 ...
 */
export const initializeUserStreak = async (req, res) => {
    try {
        const userId = req.user?._id;
        if (!userId) {
            res.status(401).json({ error: 'Unauthorized' });
 ...
 */
export const getStreakBenefits = async (req, res) => {
    try {
        res.status(200).json({
            success: true,
            data: {
...
//# sourceMappingURL=advancedStreakController.js.map
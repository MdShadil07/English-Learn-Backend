...
import { calculateCumulativeXP, getLevelFromXP } from '../services/Gamification/xpCalculator.js';
// ========================================
// SCHEMA DEFINITION
// ========================================
...
// Average skill score
progressSchema.virtual('averageSkillScore').get(function () {
...
progressSchema.pre('save', async function (next) {
    ...
    const weightedSum = Object.entries(skills).reduce((sum, [skill, value]) => {
        return sum + (value * (weights[skill] || 1));
    }, 0);
    ...
    if (this.stats.totalTimeSpent > 0) {
        this.analytics.learningVelocity = Math.round((this.totalXP / this.stats.totalTimeSpent) * 60);
    }
 ...
 */
progressSchema.methods.addXP = async function (xpAmount, category, source = 'accuracy') {
 ...
 */
progressSchema.methods.updateSkillMetrics = async function (skill, value) {
 ...
 */
progressSchema.methods.updateAccuracyData = async function (accuracyResult) {
 ...
 */
progressSchema.methods.updateCategoryProgress = async function (category, data) {
    const categoryIndex = this.categories.findIndex((c) => c.name === category);
    if (categoryIndex === -1) {
        // Create new category
 ...
 * Record a session
 */
progressSchema.methods.recordSession = async function (sessionData) {
 ...
 */
progressSchema.methods.calculateRank = async function () {
 ...
 * Get weekly report
 */
progressSchema.methods.getWeeklyReport = async function () {
    const lastWeek = this.sessionHistory.filter((session) => {
    ...
    return {
        ...
        averageAccuracy: lastWeek.reduce((sum, s) => sum + s.accuracyRate, 0) / lastWeek.length || 0,
        totalTime: lastWeek.reduce((sum, s) => sum + s.duration, 0),
        categoriesPracticed: [...new Set(lastWeek.map((s) => s.category))],
    };
};
/**
 * Get monthly report
 */
progressSchema.methods.getMonthlyReport = async function () {
    const lastMonth = this.sessionHistory.filter((session) => {
    ...
    return {
        ...
        averageAccuracy: lastMonth.reduce((sum, s) => sum + s.accuracyRate, 0) / lastMonth.length || 0,
        totalTime: lastMonth.reduce((sum, s) => sum + s.duration, 0),
        categoriesPracticed: [...new Set(lastMonth.map((s) => s.category))],
 ...
 * Get accuracy trends
 */
progressSchema.methods.getAccuracyTrends = async function (days = 7) {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - days);
    const recentHistory = this.accuracyHistory.filter((entry) => entry.date >= cutoffDate);
 ...
 */
progressSchema.methods.getLevelUpStats = async function () {
    ...
    const timeBetweenLevels = recentLevelUps.map((levelUp, index) => {
        if (index === 0)
            return null;
    ...
    const avgTime = timeBetweenLevels.length > 0
        ? timeBetweenLevels.reduce((sum, l) => sum + l.timeTaken, 0) / timeBetweenLevels.length
        : 0;
    const fastestLevelUp = timeBetweenLevels.length > 0
        ? timeBetweenLevels.reduce((fastest, current) => !fastest || current.timeTaken < fastest.timeTaken ? current : fastest, null)
        : null;
    return {
 ...
 */
progressSchema.statics.getGlobalLeaderboard = async function (limit = 100) {
 ...
 */
progressSchema.statics.getWeeklyLeaderboard = async function (limit = 100) {
    return await this.find()
        .sort({ weeklyXP: -1 })
        .limit(limit)
        .populate('userId', 'username fullName avatar')
 ...
 */
progressSchema.statics.getCategoryLeaderboard = async function (category, limit = 100) {
